...
def get_metricas_from_editorial(id, tipo):
    ...
    def merge_queries(select, table_name):
        ...
        query += f" WHERE m.editorial = (SELECT e.nombre from p_editor e WHERE e.id = %s)"

        return query

    queries = {"spi": merge_queries(select={'id': 'm.idMetrica', 'editorial': 'm.editorial', 'editorial_original': 'm.editorial_original',
                                            'año': 'm.agno', 'categoria': 'm.categoria', 'puntuacion': 'CAST(m.puntuacion AS DOUBLE)',
                                            'posicion': 'm.posicion', 'total_ed': 'm.total_ed', 'cuartil': 'm.cuartil'},
                                    table_name="m_spi",),
               "csic": merge_queries(select={'id': 'm.id', 'editorial': 'm.editorial', 'puntuacion': 'm.puntuacion', },
                                     table_name="m_csic",)}
    query = queries[tipo]
    params = [id]
    db = BaseDatos()
