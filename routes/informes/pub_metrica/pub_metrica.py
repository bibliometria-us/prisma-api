...
def generar_informe(fuentes, año_inicio, año_fin, tipo, filename):
    ...
    if tipo == "excel":
        columnas_publicaciones = datos_publicaciones(
        ...
        generar_excel(investigadores, columnas_publicaciones,
                      jif, sjr, idr, citescore, filename + ".xlsx")
...
# @timer
def generar_excel(investigadores, columnas_publicaciones, jif, sjr, idr, citescore, filename):
    ...
    for index, publicacion in enumerate(publicaciones[1:]):
        ...
        for dato in result["Prisma"]:
            if pub_indices.get(dato):
                result["Prisma"][dato][index] = publicacion[pub_indices.get(
                    dato)]

        # Datos de la página Citas
        for dato in result["Citas"]:
            if pub_indices.get(dato):
                result["Citas"][dato][index] = publicacion[pub_indices.get(
                    dato)]

        # Datos de la página JIF
        for dato in result["JIF"]:
            if pub_indices.get(dato):
                result["JIF"][dato][index] = publicacion[pub_indices.get(
                    dato)]
            if jif_indices.get(dato):
                result["JIF"][dato][index] = jif[index+1][jif_indices.get(
                    dato)]

        # Datos de la página SJR
        for dato in result["SJR"]:
            if pub_indices.get(dato):
                result["SJR"][dato][index] = publicacion[pub_indices.get(
                    dato)]
            if sjr_indices.get(dato):
                result["SJR"][dato][index] = sjr[index+1][sjr_indices.get(
                    dato)]

        # Datos de la página Dialnet
        for dato in result["Dialnet"]:
            if pub_indices.get(dato):
                result["Dialnet"][dato][index] = publicacion[pub_indices.get(
                    dato)]
            if idr_indices.get(dato):
                result["Dialnet"][dato][index] = idr[index+1][idr_indices.get(
                    dato)]

        # Datos de la página CiteScore
        for dato in result["CiteScore"]:
            if pub_indices.get(dato):
                result["CiteScore"][dato][index] = publicacion[pub_indices.get(
                    dato)]
            if citescore_indices.get(dato):
                result["CiteScore"][dato][index] = citescore[index+1][citescore_indices.get(
                    dato)]
    return result
