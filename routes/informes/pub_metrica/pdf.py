...
def generar_pdf(resumen, filename):
    ...
    def header(canvas, doc):
        ...
        # Imágenes
        canvas.drawImage(
            "routes/informes/pub_metrica/static/logo_us.png",
            (total_width - 127 * 0.5) / 2 - 220,
            700,
            width=127 * 0.5,
            height=112 * 0.5,
            mask="auto",
        ...
        canvas.drawImage(
            "routes/informes/pub_metrica/static/logo_bib.png",
            (total_width - 332 * 0.5) / 2 - 20,
            695,
            width=322 * 0.5,
            height=115 * 0.5,
            mask="auto",
        ...
        canvas.drawImage(
            "routes/informes/pub_metrica/static/logo_prisma.png",
            (total_width - 2046 * 0.08) / 2 + 200,
            710,
            width=2046 * 0.08,
            height=380 * 0.08,
            mask="auto",
    ...
    table_style_matriz_leyenda = TableStyle(table_style_matriz.getCommands())
    for propiedad in propiedades_leyenda:
    ...
    for key, value in resumen["investigadores_por_rama"].items():
        row = [value["Rama US"], value["Área de Conocimiento"], value["Miembros"]]
    ...
    datos_tabla_categorias_profesionales = [["Categoría Profesional", "Miembros"]]
    ...
    tabla_categorias_profesionales = Table(datos_tabla_categorias_profesionales)
    ...
    datos_tabla_publicaciones_por_año = [[""], ["Nº de publicaciones"]]
    ...
    for key, value in publicaciones_por_año.items():
        ...
        datos_tabla_publicaciones_por_año[1].append(cantidad_publicaciones)
    ...
    datos_tabla_publicaciones_por_tipo = [["Tipo", "Nº de publicaciones"]]
    ...
    tabla_citas_publicaciones = Table(datos_tabla_citas_publicaciones, colWidths=75)
    ...
    tabla_citas_publicaciones_grupales.setStyle(table_styles_citas_publicaciones)
            ...
            [
                ...
                KeepTogether(
                    [
                        KeepTogether(
                            [
                                Paragraph("Citas de publicaciones:", estilo_subtitulos),
                                whitespace,
                        ...
                        KeepTogether(
                            [
                                Paragraph(
                                    "Citas de publicaciones grupales:",
                                    estilo_subtitulos,
                                ),
                                whitespace,
    ...
    titulo_factores_impacto = Paragraph(
        f"Distribución de publicaciones ({resumen['año_inicio']}-{resumen['año_fin']})",
        estilo_titulos,
    )

    distribucion_publicaciones = resumen["distribucion_publicaciones"]

    datos_tabla_cuartiles = [["", "Q1", "Q2", "Q3", "Q4", "Total", "No incluidas"]]
    ...
    for key, value in distribucion_publicaciones.items():
        ...
        for cuartil, datos in cuartiles["incluidas"].items():
            if isinstance(datos, dict):
                fila_cuartiles.append(f"{datos['valor']}({datos['porcentaje']})")
            else:
        ...
        for decil, datos in deciles["incluidas"].items():
            if isinstance(datos, dict):
                fila_deciles.append(f"{datos['valor']}({datos['porcentaje']})")
            else:
        ...
        datos_tabla_deciles_d5_d10.append([fila_deciles[0]] + fila_deciles[6:11])
        datos_tabla_deciles_total.append([fila_deciles[0]] + fila_deciles[11:13])

    # Leyenda
    leyenda_distribucion_publicaciones = (
        "JIF: Journal Impact Factor; SJR: Scimago Journal & Country Rank"
    )
    datos_tabla_cuartiles.append([leyenda_distribucion_publicaciones])
    datos_tabla_deciles_total.append([leyenda_distribucion_publicaciones])
    ...
    tabla_deciles_total = Table(datos_tabla_deciles_total, colWidths=[70, 175, 175])
