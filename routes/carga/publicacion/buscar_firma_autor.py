...
class BusquedaFirmaAutor:
    ...
    def find_author_in_paper(
        self, target_name: str, paper_authors: list[str], direction: str = "forward"
    ):
        ...
        for author in paper_authors:
            ...
            score = self.monge_elkan_similarity(
                target_tokens, author_tokens, direction=direction
            )

            match_results.append({"signature": author, "score": round(score, 2)})
    ...
    def get_best_match(self, min_score=0.0):
        ...
        if (
            len(self.candidatos) > 1
            and self.candidatos[0]["score"] == self.candidatos[1]["score"]
        ):
            return None
