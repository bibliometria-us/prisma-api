...
resultado_namespace = Namespace(
    'resultado', description="Resultados de investigación")
...
columns = ["p.id", "p.tipo", "p.numero_solicitud", "DATE_FORMAT(p.fecha_solicitud,'%d/%m/%Y') as fecha_solicitud",
           "p.titulo", "p.idioma_titulo", "p.resumen", "p.visible",
           "url_invenes.valor as datos_valor", "otros_codigos.valor as datos_otros_codigos", "url_archivo.valor as datos_url_archivo",
           "fecha_concesion.valor as datos_fecha_concesion", "fecha_publicacion_so.valor as datos_fecha_publicacion_so",
           "numero_publicacion.valor as datos_numero_publicacion", "fecha_publicacion.valor as datos_fecha_publicacion",
           "titulo_alt.valor as datos_titulo_alt", "enlace.valor as datos_enlace", "url.valor as datos_url"]
...
left_joins = [plantilla_datos.format("url_invenes"),
...
class Patentes(Resource):
    ...
    def get(self):
        ...
        activos = True if (args.get('activos', "True").lower()
                           == "true") else False
        ...
        amount = int(get_resultados(
            count_columns, conditions, params)[1][0])

        longitud_pagina, offset = pages.get_page_offset(
            pagina, longitud_pagina, amount)
        try:
            data = get_resultados(columns, conditions,
                                  params, limit=longitud_pagina, offset=offset)
        except:
            resultado_namespace.abort(500, 'Error del servidor')
...
def get_materias_from_resultado(id):
    columns = ["mp.id", "mp.clasificacion", "mp.materia",
               "mp.edicion", "mcip.materia as titulo"]
    left_joins = [
...
def get_titulares_from_resultado(id):
    columns = ["tp.id", "tp.nombre",
               "CAST(tp.porcentaje AS DOUBLE) as porcentaje"]
