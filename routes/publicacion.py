...
columns = [
    ...
    "p.tipo as tipo",
    "p.agno as año",
    "fuente.tipo as fuente_tipo",
    "fuente.titulo as fuente_titulo",
    "fuente.editorial as fuente_editorial",
    "dialnet.valor as identificador_dialnet",
    "doi.valor as identificador_doi",
    "idus.valor as identificador_idus",
    "pubmed.valor as identificador_pubmed",
    "scopus.valor as identificador_scopus",
    "wos.valor as identificador_wos",
    "cod_programa.valor as datos_cod_programa",
    "congreso.valor as datos_congreso",
    "doceuropeo.valor as datos_doceuropeo",
    "fecha_lectura.valor as datos_fecha_lectura",
    "internacional.valor as datos_internacional",
    "lugar_lectura.valor as datos_lugar_lectura",
    "cod_programa.valor as datos_cod_programa",
    "nota.valor as datos_nota",
    "num_articulo.valor as datos_num_articulo",
    "numero.valor as datos_numero",
]
...
left_joins = [
    ...
    plantilla_datos.format("cod_programa"),
    plantilla_datos.format("congreso"),
    plantilla_datos.format("doceuropeo"),
    plantilla_datos.format("fecha_lectura"),
    plantilla_datos.format("internacional"),
    plantilla_datos.format("lugar_lectura"),
    plantilla_datos.format("volumen"),
    plantilla_datos.format("nota"),
    plantilla_datos.format("num_articulo"),
    plantilla_datos.format("numero"),
    plantilla_datos.format("pag_fin"),
    plantilla_datos.format("pag_inicio"),
...
def get_estadisticas_publicaciones(columns, left_joins, inactivos, conditions, params):
    table = get_publicaciones(columns, left_joins, inactivos, conditions, params)
    ...
    indexes = {
        ...
        if column == "tipo" or column.startswith("identificador_")
    }
...
class Publicaciones(Resource):
    ...
    def get(self):
        ...
        tesis = True if (args.get("tesis", "False").lower() == "true") else False
        ...
        if fuente:
            conditions.append("p.idFuente = %s ")
        ...
        if is_paginable:
            if not total_elementos:
                amount = int(
                    get_publicaciones(
                        count_prefix, _left_joins, inactivos, conditions, params
                    )[1][0]
                )
            else:
