...
from routes.carga.investigador.investigador.buscar_firma_autor import BusquedaFirmaAutor
...
def test_buscar_firma_autor():
    ...
    for _, publicacion in autores_prisma.groupby('id_publicacion', sort = False):
        ...
        for index, autor in publicacion_prueba.iterrows():
            ...
            busqueda.find_author_in_paper(nombre_autor, lista_firmas, direction='backward')
            match = busqueda.get_best_match(min_score=0.7)
            ...
            busqueda_id_investigador = publicacion_original.loc[match_condition, 'id_investigador']
    ...
    print(f"Total publicaciones: {total}, Éxitos: {exito}, Errores: {errores}, Ratio de éxito: {ratio_exito}. Puntuación mínima encontrada: {min_score:.2f}.")
